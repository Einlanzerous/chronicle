...
class UploadsApi {
  ...
  ///
  /// `idempotency_key` is minted per capture and persisted by the client BEFORE the request goes out, so an HTTP retry is a replay rather than a second memo. A replay — same key, same declaration — answers `200` with the existing session.  **A mismatch answers `409`, but not always from this call.** While a session is still open under that key, a different declaration is caught right here, before anything is transferred: this call's own session-level check, the `idempotency_key_reused` case of `UploadConflict`. Once that session has finished — its memo committed, its row cleared — the key is no longer live, and there is nothing left here to check it against: re-presenting it against different content is answered `200` (a fresh session opens), and the reuse is only caught at *finalise*, after the whole file has been transferred — the same `idempotency_key_reused` shape, but from whichever call actually finalises: ordinarily the `PATCH` that completes the transfer, but a `GET` polling a resumed session, or even this `POST` reopening one whose bytes were already fully staged, can reach it too. 
  ///
  /// Note: This method returns the HTTP [Response].
  ///
  /// Parameters:
