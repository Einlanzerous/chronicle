...
COMMENT ON COLUMN tier2.discussion_participants.last_read_seq IS
  ...
  'it never decreases (CH100) and never exceeds the thread''s last turn '
  '(CH102). Always NULL for an agent (CH101).';

COMMENT ON COLUMN tier2.discussion_participants.last_read_at IS
  'CHRN-45. When the marker last moved — the thread-list sort. Paired with '
  'last_read_seq by discussion_participants_read_pair.';
